...
export interface InputBarProps extends InputViewProps {
  ...
  leftIcon?: any;
  styleLeftIcon?: any;
  onPressLens?: Function;
  lensIcon?: any;
  altLens?: string;
  lensAccessibilityHint?: string;
}

type InputSizeTypes = 'small' | 'medium' | 'big';
  ...
  return (
    <View
      style={[
        ...
        props.styleBar,
        isFocused
          ? {borderColor: theme.colors.primary_300, borderWidth: 1}
          : null,
      ]}>
      {props.leftIcon && (
        <Icon
          ...
          style={[{marginRight: 8}, props.styleLeftIcon]}
        />
      )}
      <InputView
        {...props}
      ...
      <View style={{marginLeft: 8, flexShrink: 1, flexDirection: 'row'}}>
        ...
        {props.showLens && (
          <>
            {props.onPressLens ? (
              <TouchableOpacity
                onPress={() => props.onPressLens?.()}
                accessibilityHint={props.lensAccessibilityHint}>
                <Icon
                  alt={props.altLens ?? t('input_icon_search')}
                  style={[
                    {marginRight: 8},
                    props.actionIcon ? {marginLeft: 6} : null,
                  ]}
                  source={props.lensIcon ?? theme.drawables.general.Ic_Search}
                />
              </TouchableOpacity>
            ) : (
              <Icon
