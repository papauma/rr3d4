...
import {
  ...
  searchRecentsSlice,
} from '@src/redux/slices/searchRecentsSlice';
    ...
    if (item.markerType === TypeMarker.Stop) {
      ...
      const transportMode = selectorTransportModes.find(
        (transportMode: ITransportMode) =>
          transportMode.id === parseInt(item?.data?.transportMode),
      );
  ...
  return (
    <View
      ...
      {searchRecent?.length > 0 ? (
        <SearchCard style={[{marginTop: 16, paddingHorizontal: 0}]}>
          {searchRecent.map((element: IMarker, index: number) =>
