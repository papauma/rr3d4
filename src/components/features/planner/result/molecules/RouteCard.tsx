...
export default function RouteCard(props: RouteCardProps) {
  ...
  return (
    <TouchableOpacity
      style={[
        styles(theme).card,
        props.first ? styles(theme).cardSelected : null,
        props.style,
      ]}
      onPress={() => props.onPressCard?.(props.index)}>
      {props.first && (
        <View style={styles(theme).positionFirst}>
          <View style={styles(theme).boxFirst} accessible={true}>
            <Icon source={infoOrderCard.icon} size={18} />
            <Label style={styles(theme).titleRoute}>
              {infoOrderCard.title}
            </Label>
          </View>
        </View>
      ...
      <View style={[styles(theme).row, {justifyContent: 'space-between'}]}>
        {props.isFaster && props.routeType !== TypeRouteFilter.FAST && (
          <View style={[styles(theme).row, {flexWrap: 'nowrap'}]}>
            <Icon
                source={theme.drawables.general.Ic_Bus}
                size={16}
            />
            <Label style={[styles(theme).textTag]}>{t('planner_result_card_order_fast')}</Label>
          </View>
        )}
        {props.isWalkLess && props.routeType !== TypeRouteFilter.WALK && (
          <View style={[styles(theme).row, {flexWrap: 'nowrap'}]}>
            <Icon
                source={theme.drawables.general.Ic_Walk}
                size={18}
            />
            <Label style={[styles(theme).textTag]}>{t('planner_result_card_order_walk')}</Label>
          </View>
        )}
        {props.isTransferLess && props.routeType !== TypeRouteFilter.TRANSFER && (
          <View style={[styles(theme).row, {flexWrap: 'nowrap'}]}>
            <Icon
                source={theme.drawables.general.Ic_Transbordo}
                size={18}
            />
            <Label style={[styles(theme).textTag]}>{t('planner_result_card_order_transfer')}</Label>
          </View>
        )}
      </View>
      <RouteTimeInfo
