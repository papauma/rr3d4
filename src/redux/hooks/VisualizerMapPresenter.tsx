...
export default function VisualizerMapPresenter() {
        ...
        marker.content = (
          <View style={{alignItems: 'center'}}>
            <View
              ...
              <Icon
                source={{
                  uri: selectorIcons.find(
                    element =>
                      String(element.id) === String(marker.data?.icons?.iconId),
                  )?.contentFile,
                }}
                tint={theme.colors.primary_800}
                size={18}
              />
              {selectorZoom >= 17 && marker.data?.code ? (
                <Label
                  style={{
                    marginLeft: 6,
  ...
  async function getMarkers(
    ...
    if (!zoom || zoom < 16.5 /* || !props.precarga */) {
    ...
    stops =
      selectorFilters.transportModes?.length > 0
        ? stops.filter((stop: any) =>
            selectorFilters.transportModes.includes(
              parseInt(stop.data.transportMode),
            ),
          )
        : stops;
