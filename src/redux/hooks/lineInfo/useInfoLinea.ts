...
} from '@src/redux/services/linesService';
import {contextualSlice} from '@src/redux/slices/contextualSlice';
import {
  lineInfoState,
  lineInfoStopIdState,
  updateMarkersInfoLine,
  updatePolylinesInfoline,
  updateStopSelectedId,
} from '@src/redux/slices/lineInfoSlice';
...
const useInfoLinea = () => {
  ...
  const getLineInfo = () => {
    const linea = selectorLines.find(
      (line: ILine) => String(line.id) === String(lineSelector.id),
    );
    if (!linea) {
      return;
    }
    const transportMode = linea.transportmode;
    const icon = selectorTransportMode.find(
      (transport: ITransportMode) => transport.id === transportMode,
    )?.iconId;
  ...
  const paintInfoLinea = (infoline, datosShape, stops) => {
    ...
    const polylinesLine = preparePolyLine(
      infoline.id,
      '#' + (infoline.routeColor !== 'FFFFFF' ? infoline.routeColor : '000000'),
      4,
      datosShape,
    );
    if ([7].includes(infoline.transportmode)) {
      const polylinesLineBorde = preparePolyLine(
        'borde_' + infoline.id,
        '#FFFFFF',
        1,
        datosShape,
    ...
    const markersStops = stops.map((stop, index) => {
      return {
        ...
        data: {
          name: stop.stopName.toLowerCase(),
  ...
  useEffect(() => {
    async function getFromAPi() {
      ...
      await GetLineShapeTrip({id: lineSelector.id})
        .then(async response => {
          if (response.data.length > 0) {
            const nextTripId = lineSelector.tripId
              ? lineSelector.tripId
              : response.data[0].id;
            ...
            await GetLineDirections(lineSelector.id).then(lineDirections => {
              ...
              let direccionActual = directions.find(
                (direccion: any) => direccion.tripId === nextTripId,
              );
              if (!direccionActual) {
                direccionActual = directions.find(
                  (direccion: any) => direccion.tripId === response.data[0].id,
                );
              }
            ...
            if (nextTripId !== null) {
              await GetLineStopwithTrip({
                id: lineSelector.id,
                tripId: nextTripId,
              }).then(lineStops => {
                if (lineStops.data) {
                  let foundStopInLine = lineStops.data.find(
                    (element: any) =>
                      String(element.id) === String(stopOfLineSelected),
                  );
                  if (!foundStopInLine) {
            ...
            dispatch(
              contextualSlice.actions.updateShowLoadingBackground(false),
            );
          } else {
